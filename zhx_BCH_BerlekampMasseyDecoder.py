...
def zhx_BCH_BerlekampMasseyDecoder(received: np.ndarray, BCH_t: int, myGF2map: GF2_map):
    ...
    correction_term = myGF2map.mul( np.array(correction_term, dtype=np.int32) ,  tmp )                                                  # Ex: = [-1, symd0]            
    correction_term = myGF2map.mul( correction_term, myGF2map.poly_fresh( sigma_function_itera[max_stepdiff_indexi+1, :] )  )           # Ex: = [-1, symd0] 
    next_sigma_function = myGF2map.poly_fresh( myGF2map.add( sigma_function_itera[itera_k+1, :] ,   correction_term  ) )
    ...
    corrected_polynomial = myGF2map.poly_add(received, myGF2map.poly_addinverse(error_polynomial) )
