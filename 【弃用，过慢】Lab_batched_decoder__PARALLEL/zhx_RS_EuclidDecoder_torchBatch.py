...
def zhx_RS_EuclidDecoder_inputPoly_torchBatch(received_poly_torch: torch.Tensor, RS_t: int, myGF2map: GF2_map, IOdevice: str, mutithreads: int=8):
    ...
    def process_single_sample(sample_idx):
        received_poly_thisrow = received_poly_numpy[sample_idx, :]
        return zhx_RS_EuclidDecoder_singlerow(received_poly_thisrow, RS_t, myGF2map)
    # 多线程并行处理
    with ThreadPoolExecutor(max_workers=mutithreads) as executor:
        futures = [executor.submit(process_single_sample, idx) for idx in range(batchsize)]
        results = [future.result() for future in futures]
    # 解析结果
    corrected_polynomials = (-1) * np.ones( [batchsize, RS_n] , dtype=np.int32)
    failflags = (-1) * np.ones(batchsize, dtype=np.int32)
    for idx, (corrected_poly, failflag) in enumerate(results):
        corrected_polynomials[idx, 0:len(corrected_poly)] = corrected_poly
        failflags[idx] = failflag

    corrected_polynomials_torch = torch.from_numpy(corrected_polynomials).to(device=IOdevice, dtype=torch.int32)
    failflags_torch = torch.from_numpy(failflags).to(device=IOdevice, dtype=torch.int32)
    return corrected_polynomials_torch, failflags_torch


def zhx_RS_EuclidDecoder_inputBins_torchBatch(received_bins_torch: torch.Tensor, RS_t: int, myGF2map: GF2_map, IOdevice: str, mutithreads: int=8):
    ...
    def process_single_sample(sample_idx):
        received_bins_thisrow = received_bins_numpy[sample_idx, :]
        return zhx_RS_EuclidDecoder_inputBins_singlerow(received_bins_thisrow, RS_t, myGF2map)
    # 多线程并行处理
    with ThreadPoolExecutor(max_workers=mutithreads) as executor:
        futures = [executor.submit(process_single_sample, idx) for idx in range(batchsize)]
        results = [future.result() for future in futures]
    ...
    failflags = (-1) * np.ones(batchsize, dtype=np.int32)
    for idx, (corrected_bins, failflag) in enumerate(results):
        corrected_bins_matrix[idx, :] = corrected_bins
        failflags[idx] = failflag

    corrected_bins_matrix_torch = torch.from_numpy(corrected_bins_matrix).to(device=IOdevice, dtype=torch.int32)
    failflags_torch = torch.from_numpy(failflags).to(device=IOdevice, dtype=torch.int32)
    return corrected_bins_matrix_torch, failflags_torch
